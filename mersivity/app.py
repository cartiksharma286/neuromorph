...
def load_surgical_mesh_vertices():
    ...
    if os.path.exists(npy_path):
        _cached_surgical_mesh_vertices = np.load(npy_path)
        return _cached_surgical_mesh_vertices.copy()
...
def eeg_circuitry():
    try:
        ...
        electrodes = ['Fp1', 'F3', 'C3', 'P3', 'O1', 'Fz', 'Cz', 'Pz']
        selected = ['Fp1', 'F3', 'C3', 'Cz']
        
        electrode_profiles = {}
        np.random.seed(42)
        for el in electrodes:
            base_snr = 25.0 - (imp_level * 1.2) - (noise_level * 0.8)
            tfs_phase = float(np.random.uniform(-np.pi, np.pi))
            electrode_profiles[el] = {
                'impedance': float(max(1.0, imp_level + np.random.normal(0, 0.4))),
                'snr': float(max(2.0, base_snr + np.random.normal(0, 0.6))),
                'phase': tfs_phase,
                'active': el in selected
            }
            
        gains = {}
        optimized_snr_sum = 0.0
        for el in electrodes:
            if el in selected:
                gains[el] = 120.0
                optimized_snr_sum += electrode_profiles[el]['snr']
            else:
                gains[el] = 0.0
                
        avg_snr = float(optimized_snr_sum / 8.0) if selected else 0.0
        
        steps = 35 + int(noise_level * 20)
        loss = [float(0.8 / (i + 1) + np.random.normal(0, 0.01)) for i in range(steps)]
