...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            elif message["type"] == "run_lbm":
                # Run LBM simulation
                await run_lbm_simulation(websocket, message["data"])
...
async def run_lbm_simulation(websocket: WebSocket, params: dict):
    """
    Run standalone LBM simulation (GPU-accelerated if available)
    """
    try:
        await websocket.send_json({
            "type": "lbm_status",
            "status": "running",
            "message": "Initializing LBM solver..."
        })
        
        # Extract parameters
        stent_params = params.get("stent_params", {})
        resolution = (64, 32, 32) # Standard resolution
        ...
        try:
            ...
            await websocket.send_json({
                "type": "lbm_status",
                "status": "running",
                "message": "Using CUDA-accelerated GPU solver 🚀"
            })
            
            # Create geometry mask (simplified box for now)
            geometry = np.zeros(resolution, dtype=bool)
            # Add some obstacles to simulate stent struts
            # In production, this would rasterize the actual stent mesh
            geometry[30:34, :, :] = True 
            
            lbm_params = LBMParametersGPU(
                resolution=resolution,
                tau=0.6,
                max_steps=500
            )
            
            solver = LBMSolverGPU(lbm_params, geometry)
            results = solver.solve()
            
            velocity = results['velocity']
            
            # Calculate metrics
            max_vel = float(np.max(np.linalg.norm(velocity, axis=0)))
            avg_vel = float(np.mean(np.linalg.norm(velocity, axis=0)))
            ...
            await websocket.send_json({
                ...
                "metrics": {
                    ...
                    "wss_max": max_vel * 2.5 # Approximation
                },
                "flow_field": "Flow field data would be sent here (binary)"
            })
            ...
            await websocket.send_json({
                "type": "lbm_status",
                "status": "running",
                "message": "GPU not available. Using CPU solver..."
            })
            
            # Fallback to CPU or mock
