...
function initEventListeners() {
    ...
    // LBM Simulation button
    document.getElementById('run-lbm-btn').addEventListener('click', runLBMSimulation);
...
function runLBMSimulation() {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        updateStatus('running', 'Starting LBM...');
    } else {
        alert('Backend not connected. Cannot run GPU simulation.');
    }
}
...
function handleBackendMessage(message) {
    ...
    switch (message.type) {
        ...
        case 'lbm_status':
            updateStatus('running', message.message);
            break;

        case 'lbm_complete':
            updateMetrics({
                average_velocity: message.metrics.average_velocity,
                pressure_drop: message.metrics.wss_max // Using pressure field for WSS display
            });
            // Update WSS label temporarily
            document.getElementById('pressure-drop').nextElementSibling.textContent = "Max WSS (Pa)";

            updateStatus('success', 'LBM Simulation Complete');
