...
class MRICoilApp {
    ...
    // FEA Simulation controls
    setupFEAControls() {
        const runFeaBtn = document.getElementById('runFeaBtn');
        if (runFeaBtn) {
            runFeaBtn.addEventListener('click', () => {
                this.runFEASimulation();
            });
        }
    }

    async runFEASimulation() {
        const btn = document.getElementById('runFeaBtn');
        const img = document.getElementById('feaResultImage');
        const placeholder = document.getElementById('feaPlaceholder');
        const statsPanel = document.getElementById('feaStatsPanel');
        ...
        if (btn) {
            btn.disabled = true;
            btn.textContent = 'Running Head Coil Simulation...';
        }
        ...
        try {
            ...
            if (data.success) {
                // Update Image (Data URI, no cache busting needed)
                ...
                if (statsPanel) {
                    ...
                    document.getElementById('feaB0').textContent = `${data.stats.b0_strength.toFixed(4)} T`;
                    document.getElementById('feaB1').textContent = `${data.stats.max_b1_strength.toFixed(4)} T`;
                    document.getElementById('feaDef').textContent = data.stats.deformation;
                }
            }
        } catch (error) {
            console.error('FEA Error:', error);
            alert('Simulation failed. Make sure the backend server is running.');
        } finally {
            if (btn) {
                btn.disabled = false;
                btn.textContent = 'Run Head Coil Simulation';
            }
        }
    }
