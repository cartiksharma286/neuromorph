...
$$ \hat{I}_{i,j} = \bar{I}_{i,j} + k_{i,j} (I_{i,j} - \bar{I}_{i,j}) $$

where the weighting factor $k_{i,j}$ depends on local variance $\sigma^2$:

$$ k_{i,j} = \frac{\sigma_{i,j}^2}{\sigma_{i,j}^2 + \sigma_{noise}^2} $$

### 11.2 Non-Local Means (NLM) Denoising
The discrete NLM equation for a pixel $p$:

$$ NL(I)(p) = \frac{1}{Z(p)} \sum_{q \in \Omega} w(p, q) I(q) $$

The weights $w(p, q)$ depend on patch similarity:

$$ w(p, q) = \exp \left( -\frac{\| \mathcal{N}(p) - \mathcal{N}(q) \|_2^2}{h^2} \right) $$

where $\mathcal{N}(p)$ is the local neighborhood patch and $h$ is the decay parameter.
...
The transverse magnetic field $B_1^+(\mathbf{r})$ generated by a coil with current density $\mathbf{J}(\mathbf{r}')$ is governed by the Biot-Savart Law:

$$ B_1^+(\mathbf{r}) = \frac{\mu_0}{4\pi} \int_V \frac{\mathbf{J}(\mathbf{r}') \times (\mathbf{r} - \mathbf{r}')}{|\mathbf{r} - \mathbf{r}'|^3} dV' $$

For a **Birdcage Coil** (Quadrature mode), the idealized field is homogeneous:

$$ B_1^+ = \frac{\mu_0 I N}{2D} \hat{x} + i \frac{\mu_0 I N}{2D} \hat{y} $$

For a **Surface Loop Coil** of radius $a$ at height $z$:

$$ B_z(z) = \frac{\mu_0 I a^2}{2(a^2 + z^2)^{3/2}} $$

### 12.2 Pulse Sequence Excitation Profiles
The local flip angle $\alpha(\mathbf{r})$ induced by a pulse sequence of duration $\tau$ depends on the coil's sensitivity profile $S(\mathbf{r})$:

$$ \alpha(\mathbf{r}) = \gamma \int_0^\tau |B_1^+(\mathbf{r}, t)| dt \approx \gamma \tau |S(\mathbf{r})| V_{pulse} $$

**12.2.1 Spin Echo (SE) in Surface Coils**
The refocusing ($180^\circ$) pulse is highly sensitive to $B_1^+$ inhomogeneity. The effective signal $M_{xy}$ is:

$$ M_{xy}(\mathbf{r}) = M_0 \sin^3(\alpha(\mathbf{r})) \cdot e^{-TE/T2} $$

This cubic dependence implies rapid signal drop-off away from the coil surface.

**12.2.2 Gradient Echo (GRE) Optimization**
To maximize SNR given coil sensitivity $S(\mathbf{r})$, the local Ernst Angle is:

$$ \alpha_E(\mathbf{r}) = \arccos(e^{-TR/T1}) $$

Since $\alpha$ is fixed by the transmit coil, local signal strength varies as:

$$ M_{GRE}(\mathbf{r}) = M_0 \frac{(1-e^{-TR/T1})\sin(\alpha(\mathbf{r}))}{1 - \cos(\alpha(\mathbf{r}))e^{-TR/T1}} \cdot S^*(\mathbf{r}) $$

### 12.3 Quantum Coil-Sequence Coupling
For **Quantum Berry Phase sequences**, the phase accumulation $\Phi_B$ is topologically protected against linear coil inhomogeneities ($\nabla B_1$). The coupling Hamiltonian $H_{int}$ is:

$$ H_{int} = -\gamma \mathbf{S} \cdot (\mathbf{B}_0 + \mathbf{B}_{RF}(t)) $$

The geometric phase acquired is proportional to the solid angle $\Omega$ swept by the effective field vector $\mathbf{B}_{eff}$ generated by the coil array:

$$ \Phi_B = -\frac{1}{2} \Omega(C) = -\frac{1}{2} \int_S \nabla \times \mathbf{A}_{coil} \cdot d\mathbf{S} $$

This implies that "Quantum" sequences are robust against the $B_1$ drop-off characteristic of surface coils, maintaining contrast fidelity even at depth.

---

## Conclusion

This report provides the complete finite mathematical framework for quantum phase MRI simulation. It establishes the rigorous discrete formalism for:
1.  **Bloch Dynamics**: Finite difference time-domain evolution of magnetization.
2.  **Topological Physics**: Berry phase accumulation and Chern number invariants.
3.  **Hardware Modeling**: Discrete impedance matrices for Birdcage and Surface coils.
4.  **Reconstruction**: SENSE parallel imaging and Quantum-Statistical artifact removal.
5.  **Pulse-Coil Interaction**: Coupled equations describing excitation inhomogeneity and quantum protection.

All equations are presented in forms directly implementable in the NeuroPulse simulation engine (Python/NumPy), ensuring physically accurate modeling of quantum topological effects in magnetic resonance imaging.
...
5. Perona, P. & Malik, J. (1990). "Scale-space and edge detection using anisotropic diffusion"
6. Buades, A. et al. (2005). "A non-local algorithm for image denoising"
7. Hoult, D. I. (2000). "The Principle of Reciprocity in NMR Microscopy"
