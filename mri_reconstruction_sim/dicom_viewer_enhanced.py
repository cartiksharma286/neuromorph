#!/usr/bin/env python3
"""
Enhanced DICOM Neuroimage Viewer with Auto-Load and Demo Mode
Features:
...
- Keyboard shortcuts for easy testing
"""
...
class DICOMLoader(QThread):
    ...
    def __init__(self, file_path):
        ...
        # Make thread non-blocking cleanup by default
...
class DICOMNeuroimageViewer(QMainWindow):
    ...
    def __init__(self, auto_load_file=None):
        super().__init__()
        self.setWindowTitle("DICOM Neuroimage Viewer - PyQt5 + ITK + VTK (Enhanced)")
        ...
        # Track active threads to prevent semaphore leaks
        self.active_threads = []
        ...
        if auto_load_file and os.path.exists(auto_load_file):
            QTimer.singleShot(500, lambda: self.load_dicom_internal(auto_load_file))
        
    def create_status_bar(self):
        """Create status bar"""
        self.statusBar = QStatusBar()
        self.setStatusBar(self.statusBar)
        self.statusBar.showMessage("Ready | Ctrl+O: Load DICOM | Ctrl+D: Demo | Ctrl+Q: Quit")
    ...
    def create_menu(self):
        ...
        demo_action = QAction("Run Auto Demo", self)
    ...
    def create_control_panel(self):
        ...
        # Gaussian
        process_layout.addWidget(QLabel("Gaussian Smoothing (σ):"))
    ...
    def load_dicom_internal(self, source):
        ...
        self.statusBar.showMessage(f"Loading {source}...")
        ...
        # Connect finished signal to clean up thread reference
    ...
    def on_dicom_loaded(self, data):
        ...
        self.update_info_label(pixel_array)
        self.update_display()
        self.progress_bar.setVisible(False)
        
        self.statusBar.showMessage(f"Successfully loaded DICOM: {pixel_array.shape}")
    ...
    def update_info_label(self, image_array):
        """Update image information label"""
        if image_array is not None:
            info_text = f"""Image: {image_array.shape}
Min: {image_array.min():.0f}
Max: {image_array.max():.0f}
...
Std: {image_array.std():.1f}"""
    ...
    def reset_processing(self):
        ...
        self.threshold_slider.setValue(128)
        self.update_display()
        self.statusBar.showMessage("Processing reset to original image")
    ...
    def render_3d_volume(self):
        ...
        try:
            ...
            self.statusBar.showMessage("3D Volume rendering displayed")
        except Exception as e:
    ...
    def render_3d_surface(self):
        ...
        try:
            ...
            self.statusBar.showMessage(f"3D Surface rendering displayed (threshold={threshold})")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Could not render 3D surface: {str(e)}")
    
    def run_demo(self):
        """Run automatic demo of features"""
        if self.current_image is None:
            QMessageBox.warning(self, "No Data", "Please load a DICOM file first")
            return
        
        QMessageBox.information(self, "Demo Mode", "Starting automatic demo...\n\nSlider will automatically cycle through different processing effects")
        
        # Demo: cycle through Gaussian values
        for sigma in [0.5, 1.0, 1.5, 2.0, 1.5, 1.0, 0.5]:
            self.gaussian_slider.setValue(int(sigma * 10))
            QApplication.processEvents()
            threading.Event().wait(0.5)
        
        # Reset
        self.reset_processing()
        QMessageBox.information(self, "Demo Complete", "Automatic demo finished!")
...
Ctrl+D: Run auto demo
Ctrl+Q: Quit application

Left/Right Arrow: Navigate slices
Up/Down Arrow: Adjust Gaussian sigma
"""
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Left:
            # Previous slice
            self.slice_slider.setValue(max(0, self.current_slice - 1))
        elif event.key() == Qt.Key_Right:
            # Next slice
            self.slice_slider.setValue(min(self.slice_slider.maximum(), self.current_slice + 1))
        elif event.key() == Qt.Key_Up:
            # Increase Gaussian
            self.gaussian_slider.setValue(min(50, self.gaussian_slider.value() + 5))
        elif event.key() == Qt.Key_Down:
            # Decrease Gaussian
            self.gaussian_slider.setValue(max(0, self.gaussian_slider.value() - 5))
        else:
...
def main():
    try:
        # Check for auto-load DICOM file
        auto_load_file = "/Users/cartiksharma/Downloads/neuromorph-main-10/mri_reconstruction_sim/neurovascular_brain_with_tumor.dcm"
        
        app = QApplication(sys.argv)
        viewer = DICOMNeuroimageViewer(auto_load_file=auto_load_file if os.path.exists(auto_load_file) else None)
        viewer.show()
        print("=" * 70)
        print("✓ DICOM Neuroimage Viewer (Enhanced) started successfully")
        print("=" * 70)
        print("\nKeyboard Controls:")
        print("  Left/Right Arrow: Navigate slices")
        print("  Up/Down Arrow: Adjust Gaussian sigma")
        print("  Ctrl+O: Open DICOM file")
        print("  Ctrl+D: Run demo")
        print("  Ctrl+Q: Quit")
        print("\nSample DICOM file will auto-load from:")
        print(f"  {auto_load_file}")
        print("=" * 70)
        
        # Run application and ensure clean exit
        exit_code = app.exec_()
