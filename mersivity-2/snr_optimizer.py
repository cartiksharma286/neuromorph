...
class SNROptimizer:
    ...
    def _denoise_signal(self, noisy_signal: np.ndarray, params: Dict) -> np.ndarray:
        ...
        if self.distribution_type == 'gaussian':
            ...
            # Soft-thresholding shrinkage based on Laplace distribution parameters
            mu = params['mu']
            b = params['b']
            
            # Median-based noise estimate
            noise_std = np.median(np.abs(noisy_signal - np.median(noisy_signal))) / 0.6745
            threshold = np.sqrt(2) * (noise_std ** 2) / (b + 1e-8)
            
            # Perform soft thresholding on deviations from mu
            dev = noisy_signal - mu
            denoised_dev = np.sign(dev) * np.maximum(0, np.abs(dev) - threshold)
            return mu + denoised_dev
            
        elif self.distribution_type == 'student_t':
            # Student-t robust shrinkage: heavy-tailed signals (outliers/spikes)
            loc = params['loc']
            scale = params['scale']
            df = params['df']
            
            # Robust weight function: downweights large deviations
            dev = noisy_signal - loc
            noise_std = np.median(np.abs(noisy_signal - np.median(noisy_signal))) / 0.6745
            
            # Weight is based on student-t PDF ratio or robust influence function
            # w = (df + 1) / (df + (dev/scale)^2)
            weights = (df + 1.0) / (df + (dev / (scale + 1e-8))**2)
            
            # Shrinkage: denoised signal is weighted deviation from loc
            # Higher weights for points close to loc (true signal), lower weights for outliers
            gain = (scale**2) / (scale**2 + noise_std**2)
            denoised = loc + gain * weights * dev
            return denoised
