...
def create_premium_material(name, diffuse_color, roughness=0.5, metallic=0.0, transmission=0.0, emission_color=None, emission_strength=1.0):
    ...
    if emission_color:
        ec_rgba = list(emission_color)
        if len(ec_rgba) == 3:
            ec_rgba.append(1.0)
        ec_rgba = tuple(ec_rgba)
        ec_rgb = ec_rgba[:3]

        if 'Emission Color' in node_principled.inputs:
            try:
                node_principled.inputs['Emission Color'].default_value = ec_rgba
            except Exception:
                node_principled.inputs['Emission Color'].default_value = ec_rgb
        elif 'Emission' in node_principled.inputs:
            try:
                node_principled.inputs['Emission'].default_value = ec_rgba
            except Exception:
                node_principled.inputs['Emission'].default_value = ec_rgb
...
def setup_lighting_and_camera():
    ...
    cam_obj.location = (240.0, -480.0, 320.0)
...
def main():
    ...
    materials = {
        ...
        # New Registration Materials
        'reg_mri_to_ct_qml': create_premium_material('Mat_RegMriToCtQml', (0.05, 0.9, 0.45, 1.0), roughness=0.2, metallic=0.8, emission_color=(0.05, 0.9, 0.45, 1.0), emission_strength=1.8),
        'reg_mri_to_stl_qml_feynman': create_premium_material('Mat_RegMriToStlQmlFeynman', (0.8, 0.0, 0.9, 1.0), roughness=0.1, metallic=0.9, emission_color=(0.8, 0.0, 0.9, 1.0), emission_strength=2.2),
    ...
    models_config = [
        ...
        # --- NEW MRI TO CT / STL REGISTRATIONS & SUPERIMPOSED ---
        ('registered_mri_to_ct_qml.stl', col_registration, materials['reg_mri_to_ct_qml'], (120.0, 300.0, 0.0)),
        ('registered_mri_to_stl_qml_feynman.stl', col_registration, materials['reg_mri_to_stl_qml_feynman'], (360.0, 300.0, 0.0)),
