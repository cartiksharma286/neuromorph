...
def create_dicom_slice(filename, pixel_array, instance_number, series_uid, study_uid, series_number, series_description, rows=256, cols=256):
    """Creates a highly compliant high-resolution 256x256 DICOM slice."""
    file_meta = Dataset()
    ...
    ds.PatientName = "Atlas^Neurovascular"
    ds.PatientID = "ATLASVASC001"
    ...
    # Set high-resolution dimensions (256 x 256)
    ds.Rows = rows
    ds.Columns = cols
    ds.PixelSpacing = [0.8, 0.8]  # high resolution pixel spacing
...
def simulate_atlas_volume(nx=256, ny=256, nz=30, contrast_type="T1", segments=None):
    """
    Simulates a high-resolution 256x256x30 Atlas Brain structure + Vascular network.
    - contrast_type = "T1": high-contrast anatomical scan (CSF dark, WM bright).
    - contrast_type = "MRA": magnetic resonance angiography (brain heavily suppressed, vessels bright).
    """
    ...
    elif contrast_type == "MRA":
        # Heavily suppressed background brain tissue for angiography
        volume.fill(8.0)
        volume[r_head < 0.96] = 45.0
        volume[r_brain < 0.88] = 60.0
        volume[r_wm < 0.74] = 75.0
        volume[ventricles < 1.0] = 30.0
    ...
    if segments is not None:
        ...
        # Render vascular tree based on contrast type
        if contrast_type == "T1":
            # Flow voids / moderate enhancement in anatomical reference
            # Make vessels appear as distinct flow void boundaries or bright MRA-ref contrast
            volume = (1.0 - 0.25 * vessel_mask) * volume + 1800.0 * vessel_mask
        elif contrast_type == "MRA":
            # Bright high-intensity angiographic signal (Circle of Willis glowing)
            volume += 3500.0 * vessel_mask
...
def generate_neurovascular_dataset():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    study_dir = os.path.join(base_dir, 'mri', 'DICOM', 'atlas_neurovascular')
    os.makedirs(study_dir, exist_ok=True)
    
    # Generate unique Study Instance UID
    study_uid = generate_uid()
    print(f"Generating high-resolution Atlas Neurovascular study with StudyInstanceUID: {study_uid}")

    # Main cerebral arteries segments (representing Basilar, ICAs, and Circle of Willis branches)
    segments = [
        # Basilar Artery (BA)
    ...
    # =========================================================================
    # SERIES 1: High-resolution anatomical Atlas T1 reference scan
    # =========================================================================
    t1_dir = os.path.join(study_dir, 'Series_01_Atlas_T1')
    os.makedirs(t1_dir, exist_ok=True)
    t1_series_uid = generate_uid()
    
    print("\n--- Generating Series 1: Anatomical Atlas T1 (256 x 256 x 30) ---")
    t1_volume = simulate_atlas_volume(nx=256, ny=256, nz=30, contrast_type="T1", segments=segments)
    ...
    for i in range(30):
        ...
        create_dicom_slice(
            ...
            study_uid=study_uid,
            series_number=1,
            series_description="Atlas Structural T1 Reference"
        )
    print(f"Series 1 successfully written to: {t1_dir}")

    # =========================================================================
    # High-resolution Laser Scan STL mapping to the scalp boundary (256x256 space)
    # =========================================================================
    print("Generating high-resolution laser scan STL from structural volume...")
    # Isovalue at 320.0 cleanly isolates outer skin/scalp boundary
    ...
    # =========================================================================
    # SERIES 2: High-resolution MRA Vascular reference scan
    # =========================================================================
    mra_dir = os.path.join(study_dir, 'Series_02_MRA_Vascular')
    os.makedirs(mra_dir, exist_ok=True)
    mra_series_uid = generate_uid()
    
    print("\n--- Generating Series 2: MRA Vascular tree (256 x 256 x 30) ---")
    mra_volume = simulate_atlas_volume(nx=256, ny=256, nz=30, contrast_type="MRA", segments=segments)
    
    for i in range(30):
        filename = os.path.join(mra_dir, f"slice_{i+1:02d}.dcm")
        slice_data = mra_volume[:, :, i]
        create_dicom_slice(
            filename=filename,
            pixel_array=slice_data,
            instance_number=i + 1,
            series_uid=mra_series_uid,
            study_uid=study_uid,
            series_number=2,
            series_description="Magnetic Resonance Angiography (MRA) Willis"
        )
    print(f"Series 2 successfully written to: {mra_dir}")
    print("\n=========================================================================")
    print("Atlas Neurovascular DICOM study & laser scan STL successfully generated.")
    print("=========================================================================")

if __name__ == "__main__":
    generate_neurovascular_dataset()
