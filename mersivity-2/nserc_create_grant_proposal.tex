...
Let $Y_l^m(\theta, \phi)$ denote the spherical harmonics:
$$
...
$$
\mathcal{L}(\Theta) = -\sum_{i=1}^N \log \sum_{k=1}^K \pi_k \mathcal{N}(y_i | \mu_k, \Sigma_k)
$$
where $\mathcal{N}$ is the Gaussian density, $\pi_k$ are mixture weights, and $\Theta$ are model parameters.
...
We provide computational proofs that the registration mapping $f: S^2 \to S^2$ is diffeomorphic, i.e., $f$ is smooth and invertible with a smooth inverse. This is verified by checking the Jacobian determinant $\det J_f > 0$ everywhere on the surface.
...
$$
\text{TRE} = \frac{1}{N} \sum_{i=1}^N \| f(x_i) - y_i \|
$$
...
*Figure 1: Home screen of the neuroimaging registration app.*

![Registration Result](registration_result.png)

*Figure 2: Visualization of registered cortical surface and surgical mesh.*
